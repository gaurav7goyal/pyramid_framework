"""My Application entry point and configuration."""
import websauna.system


class Initializer(websauna.system.Initializer):
    """An initialization configuration used for starting My Application.

    Override parent class methods to customize new_application behavior.
    """
    ...
    def configure_static(self):
        ...
        self.config.registry.static_asset_policy.add_static_view('my.new_app-static', 'my.new_app:static')

    def configure_templates(self):
        """Include our package templates folder in Jinja 2 configuration."""
        super(Initializer, self).configure_templates()

        self.config.add_jinja2_search_path('my.new_app:templates', name='.html', prepend=True)  # HTML templates for pages
        self.config.add_jinja2_search_path('my.new_app:templates', name='.txt', prepend=True)  # Plain text email templates (if any)
        self.config.add_jinja2_search_path('my.new_app:templates', name='.xml', prepend=True)  # Sitemap and misc XML files (if any)

    def configure_views(self):
        """Configure views for your new_application.
    ...
    def configure_models(self):
        """Register the models of this new_application."""
        from . import models
